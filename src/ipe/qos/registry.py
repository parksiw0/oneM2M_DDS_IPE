...
from ipe.qos import deadline, durability, history, lifespan, liveliness, reliability
from ipe.qos.models import QoSSpec

ACTIVE_POLICIES = (reliability, durability, history, deadline, lifespan, liveliness)
...
DDS_QOS_POLICIES: tuple[DDSPolicyDefinition, ...] = (
    DDSPolicyDefinition("LIFESPAN", "A", ("DATAWRITER", "TOPIC"), lifespan.DEFAULT_MAPPING),
    DDSPolicyDefinition(
        "HISTORY", "A", ("DATAWRITER", "DATAREADER", "TOPIC"), history.DEFAULT_MAPPING
    ),
    DDSPolicyDefinition(
        "RESOURCE_LIMITS",
        "A",
        ...
        {
            "kind": "RESOURCE_PARTIAL",
            "resourceType": "container",
            "attributes": ["maxNrOfInstances", "maxByteSize"],
        },
    ),
    DDSPolicyDefinition(
        "RELIABILITY", "B", ("DATAWRITER", "DATAREADER", "TOPIC"), reliability.DEFAULT_MAPPING
    ),
    DDSPolicyDefinition(
        "DEADLINE", "B", ("DATAWRITER", "DATAREADER", "TOPIC"), deadline.DEFAULT_MAPPING
    ),
    DDSPolicyDefinition(
        "LIVELINESS", "B", ("DATAWRITER", "DATAREADER", "TOPIC"), liveliness.DEFAULT_MAPPING
    ),
    DDSPolicyDefinition(
        "OWNERSHIP",
        "B",
    ...
    DDSPolicyDefinition(
        "DURABILITY", "B", ("DATAWRITER", "DATAREADER", "TOPIC"), durability.DEFAULT_MAPPING
    ),
    DDSPolicyDefinition(
        "TIME_BASED_FILTER",
        "B",
    ...
    DDSPolicyDefinition(
        "DURABILITY_SERVICE", "C", ("TOPIC",), {"kind": "METADATA", "attribute": "ddsQoSProperties"}
    ),
    DDSPolicyDefinition(
        "PRESENTATION",
        "C",
    ...
    DDSPolicyDefinition(
        "TOPIC_DATA", "C", ("TOPIC",), {"kind": "METADATA", "attribute": "ddsQoSProperties"}
    ),
    DDSPolicyDefinition(
        "USER_DATA",
        "C",
...
def rmw_policy_values(spec: QoSSpec | None) -> dict[str, Any]:
    if spec is None:
        return {}
    return {policy.NAME: policy.value(spec) for policy in ACTIVE_POLICIES}
