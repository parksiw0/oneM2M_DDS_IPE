...
def binding_map(rc: ResolvedConfig) -> dict[PlanKey, Any]:
    ...
    out.update({("topic", x.robot_id, x.interface): x for x in rc.topics if x.msg_type})
    out.update({("service", x.robot_id, x.interface): x for x in rc.services if x.srv_type})
    out.update({("action", x.robot_id, x.interface): x for x in rc.actions if x.action_type})
    return out


def append_spec(rc: ResolvedConfig, key: PlanKey, spec: Any) -> None:
    kind = key[0]
    target = rc.topics if kind == "topic" else rc.services if kind == "service" else rc.actions
